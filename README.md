# PIC-Challenges

## Inspect HTML ===> SCORE 100 "F12 and find comment"

## Password Profiler ==> SCORE 100 "Generate password from user info by cupp and compare password with hash using python script"

## First Find ==> SCORE 100 "Looking for a secret file inside folder"

## runme.py => SCORE 100 "Run a specific python file"

## repetitions => SCORE 100  decode incode text reapeated I did it by "CyberChef" tool

## Lets Warm Up => SCORE 50 Convert  0x70 to text "I Converted it to binary after that from binary to ASCII"

## Crack the Gate 1 ==> SCORE 100 "Its need to edit request of website and add to headers { X-Dev-Access: yes } and check the payload of response"
